 ...
 */
	...
	@RequiresPermissions("tclass:info")
	...
	@ResponseBody
	@RequestMapping("/save")
	@RequiresPermissions("tclass:save")
	...
	@RequiresPermissions("tclass:update")
	...
	@RequiresPermissions("tclass:delete")
	...
	public R uploadFile(@RequestBody TClassSchedule tClassSchedule){
		//判断是否上传课件
		Integer e=	tClassScheduleService.selectByScheduleId(tClassSchedule.getScheduleId());
		if(e==1){
			return R.error(4,"当前课件已经上传,请不要重复上传！！");
		}
		//1.根据ChapterId 获取文件路径
		Integer chapterId = tClassSchedule.getChapterId();
		List<TLibrary> UrlList=	tLibraryChapterService.selectFile(chapterId);
		if(UrlList==null){
			//为null 说明没有关联文件
			return R.error(2,"请关联课件后上传！！");
		}
		//2.根据文件路径进行文件下载
		Map<String,byte[]> map =	tLibraryChapterService.downloadStream(UrlList);
		Map<String,InputStream> nameStream=new HashMap<>();
		for(String s:map.keySet()){
			byte[] bytes = map.get(s);
			InputStream is=new ByteArrayInputStream(bytes);
			String fileName = s.substring(s.lastIndexOf("-")+1);
			nameStream.put(fileName,is);
		}
		//4.封装参数调用接口即可
		Integer code=talkCloudService.uploadFileByRoomId(tClassSchedule.getRoomId(),nameStream,tClassSchedule.getScheduleId().intValue());
		if(code!=0){
			return R.error(3,"文件上传失败请联系管理员");
		}
		return R.ok();
	}
